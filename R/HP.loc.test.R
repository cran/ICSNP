...
HP.loc.tyler.test <- function(X, mu = NULL, score = "rank", method = "approximation", n.perm = 1000)
    {
    k <- dim(X)[2]
    ...
    n<- length(ranksH0)
    scoresH0 <-   switch(score, sign = 1, rank = ranksH0, normal = sqrt(qchisq(ranksH0/(n+1), k)))
    signsH0w <- scoresH0 * signsH0


    switch(score, sign = {DIV <- k/n
              METHOD <- "TYLER ANGLES SIGN TEST"
              S.name <- "Q.S"}
              , 
              rank = {DIV <- 3*k / (n * (n+1)^2)
    ...
    SUMS <- matrix(0,n,n)
    for (i in 1:n) SUMS[i,] <- rowSums(sweep(signsH0w, 2, signsH0w[i, ], "*"))
    Q1 <- sum(SUMS)

    STATISTIC <- DIV * Q1


    switch(method, approximation = {PVAL <-1 - pchisq(STATISTIC, k)
                                PARAMETER <- k
                                names(PARAMETER) <- "df"}
                ,
                permutation =   {rep.func <- function(index, signs, n)
                                {
                                SUMS.2 <- matrix(0,n,n)
                                for (i in 1:n) SUMS.2[i,] <- rowSums(sweep(signs, 2, signs[i, ], "*"))
                                sum(SUMS.2)
                                }
                             Q.simu <- replicate(n.perm, rep.func(sample(c(1,-1), n, replace = TRUE), signsH0w, n))
                             PVAL <- mean(Q.simu>Q1)
                             PARAMETER<-n.perm
    ...
    res <- list(statistic = STATISTIC, p.value = PVAL, method = METHOD, parameter = PARAMETER)
    return(res)
    }
